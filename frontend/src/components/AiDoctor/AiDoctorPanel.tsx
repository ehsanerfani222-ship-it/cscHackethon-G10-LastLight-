  ...
  return (
    <motion.button
      ...
      style={{
        ...
        border: `1px solid ${color ?? 'rgba(255,255,255,0.1)'}50`,
      }}
    >
      {children}
    </motion.button>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-2xl p-4 space-y-2" style={{ background: 'rgba(0,229,255,0.05)', border: '1px solid rgba(0,229,255,0.15)' }}>
        ...
        {result.immediateSteps.map((item, index) => (
          <div key={item} className="flex items-start gap-2 text-sm">
            <span className="text-cyan-400 font-bold w-5 flex-shrink-0">{index + 1}.</span>
...
export function AiDoctorPanel() {
  ...
  const consult = async () => {
    setIsLoading(true);
    try {
      const response = await consultAIDoctor({ category, symptom, bodyArea, severity, age, additionalInfo });
      setResult(response);
      setStep('result');
      await loadHistory();
    } catch {
        ...
        <div className="mt-6 space-y-3">
          ...
          {selectedConsultation && (
            <div className="rounded-2xl p-4 space-y-3" style={{ background: 'rgba(0,229,255,0.04)', border: '1px solid rgba(0,229,255,0.15)' }}>
              ...
              <ResultCard result={selectedConsultation} />
              <textarea
                value={editingInfo}
