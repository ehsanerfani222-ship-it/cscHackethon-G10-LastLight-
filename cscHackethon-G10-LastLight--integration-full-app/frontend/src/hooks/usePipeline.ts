...
import { fetchCrises } from '../services/api';

let socket: Socket | null = null;

export function usePipeline() {
  const { selectedDate, setCrises, setPipelineState, setPredictions, addNotification } = useStore();
  const prevIdsRef = useRef<Set<string>>(new Set());
  const selectedDateRef = useRef(selectedDate);

  useEffect(() => {
    selectedDateRef.current = selectedDate;
    fetchCrises(selectedDate)
      .then(setCrises)
      .catch(() => toast.error('Unable to load crisis records for selected date'));
  }, [selectedDate, setCrises]);
  ...
  useEffect(() => {
    ...
    socket.on('crisis:update', (data: { crises: Crisis[]; pipelineState: PipelineState; predictions: Prediction[] }) => {
      ...
      data.crises
        .filter((c) => selectedDateRef.current === today && c.severity >= 7 && !prevIds.has(c.id))
        .forEach((c) => {
  ...
  }, [addNotification, setCrises, setPipelineState, setPredictions]);
}
