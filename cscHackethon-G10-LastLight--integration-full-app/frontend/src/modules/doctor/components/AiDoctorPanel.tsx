...
const DEFAULT_BODY_AREAS = ['Head / face', 'Chest / lungs', 'Abdomen', 'Back / spine', 'Arms / hands', 'Legs / feet', 'Skin', 'Whole body'];
const BODY_AREAS_BY_SYMPTOM: Record<string, string[]> = {
  cut_bleeding: ['Skin', 'Arms / hands', 'Legs / feet', 'Head / face', 'Chest / torso', 'Abdomen'],
  burn: ['Skin', 'Arms / hands', 'Legs / feet', 'Face / airway', 'Chest / torso', 'Eyes'],
  fracture: ['Arms / hands', 'Legs / feet', 'Back / spine', 'Chest / ribs', 'Hip / pelvis', 'Head / face'],
  head_injury: ['Head / face', 'Neck', 'Eyes / vision', 'Whole body'],
  sprain: ['Ankle / foot', 'Knee', 'Wrist / hand', 'Shoulder', 'Back'],
  crush_injury: ['Arms / hands', 'Legs / feet', 'Chest / torso', 'Abdomen', 'Whole body'],
  high_fever: ['Whole body', 'Head / neck', 'Chest / lungs', 'Abdomen', 'Skin'],
  breathing_difficulty: ['Chest / lungs', 'Throat / airway', 'Whole body'],
  severe_pain: ['Chest', 'Abdomen', 'Head', 'Back', 'Arms / hands', 'Legs / feet'],
  vomiting: ['Abdomen', 'Whole body', 'Head'],
  rash: ['Skin', 'Face', 'Arms / hands', 'Legs / feet', 'Whole body'],
  dizziness: ['Head', 'Chest', 'Whole body'],
  panic_attack: ['Chest', 'Head', 'Whole body'],
  shock: ['Whole body', 'Head', 'Chest'],
  suicidal: ['Mental health / safety', 'Whole body'],
  ptsd: ['Mental health / safety', 'Whole body'],
  grief: ['Mental health / safety', 'Chest', 'Whole body'],
  cardiac_arrest: ['Chest / heart', 'Whole body'],
  choking: ['Throat / airway', 'Chest / lungs'],
  drowning: ['Chest / lungs', 'Whole body'],
  poisoning: ['Mouth / throat', 'Abdomen', 'Chest / lungs', 'Whole body'],
  anaphylaxis: ['Throat / airway', 'Face / lips', 'Skin', 'Chest / lungs', 'Whole body'],
  stroke: ['Face', 'Arm', 'Speech / cognition', 'Whole body'],
  smoke_inhalation: ['Chest / lungs', 'Throat / airway', 'Eyes', 'Whole body'],
  radiation_exposure: ['Whole body', 'Skin', 'Eyes'],
  chemical_exposure: ['Skin', 'Eyes', 'Chest / lungs', 'Mouth / throat', 'Whole body'],
  heatstroke: ['Whole body', 'Head', 'Skin'],
  hypothermia: ['Whole body', 'Hands / feet', 'Head'],
  dehydration: ['Whole body', 'Mouth / throat', 'Head'],
};

const DETAIL_SUGGESTIONS: Record<string, string[]> = {
  cut_bleeding: ['Bleeding has not stopped after pressure', 'Blood is spurting', 'Object is stuck in wound', 'Tetanus status unknown'],
  burn: ['Burn is larger than palm', 'Face or airway involved', 'Chemical or electrical burn', 'Skin looks white or charred'],
  fracture: ['Limb looks deformed', 'Cannot bear weight', 'Numbness or blue fingers/toes', 'Open wound near bone'],
  head_injury: ['Lost consciousness', 'Repeated vomiting', 'Worsening headache', 'Taking blood thinners'],
  breathing_difficulty: ['Cannot speak full sentences', 'Blue lips', 'Has asthma/COPD', 'Started after smoke or allergy'],
  high_fever: ['Stiff neck', 'Rash present', 'Signs of dehydration', 'Fever over 40 C / 104 F'],
  panic_attack: ['Chest pain present', 'First episode', 'No injury or substance use', 'Has calming support nearby'],
  suicidal: ['Has a plan or means', 'Not alone right now', 'Weapons/medicines removed', 'Crisis contact called'],
  choking: ['Cannot cough or speak', 'Object cleared', 'Still coughing after choking', 'Infant or small child'],
  poisoning: ['Substance and amount known', 'Time of exposure known', 'Vomiting or confused', 'Poison control contacted'],
  anaphylaxis: ['Epinephrine available', 'Swelling lips/tongue', 'Wheezing', 'Known allergy'],
  stroke: ['Face drooping', 'Arm weakness', 'Speech trouble', 'Time symptoms started known'],
  chemical_exposure: ['Rinsing for 15 minutes', 'Contaminated clothing removed', 'Chemical name known', 'Breathing symptoms'],
  heatstroke: ['Hot and confused', 'Stopped sweating', 'Cooling started', 'Exercising or high heat exposure'],
  hypothermia: ['Wet clothing removed', 'Shivering stopped', 'Confused or drowsy', 'Warm shelter available'],
  dehydration: ['Very little urine', 'Dizzy on standing', 'Vomiting/diarrhea', 'Oral fluids tolerated'],
};
...
function getSymptomLabel(category: string, symptom: string) {
  return SYMPTOMS[category]?.find((item) => item.value === symptom)?.label ?? formatLabel(symptom);
}

function getBodyAreas(symptom: string) {
  return BODY_AREAS_BY_SYMPTOM[symptom] ?? DEFAULT_BODY_AREAS;
}
...
export function AiDoctorPanel() {
  ...
  const currentBodyAreas = getBodyAreas(symptom);
  const detailSuggestions = DETAIL_SUGGESTIONS[symptom] ?? [
  ...
  const appendDetail = (detail: string) => {
    setAdditionalInfo((current) => {
      ...
      return current.trim() ? `${current.trim()}; ${detail}` : detail;
    });
  };
        ...
        <AnimatePresence mode="wait">
          {step === 'category' && (
            <motion.div key="cat" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-3">
              ...
              {CATEGORIES.map((item) => (
                <OptionButton key={item.value} onClick={() => { setCategory(item.value); setSymptom(''); setBodyArea(''); setStep('symptom'); }} color="#00E5FF">
            ...
            <motion.div key="sym" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                {(SYMPTOMS[category] ?? []).map((item) => (
                  <OptionButton key={item.value} onClick={() => { setSymptom(item.value); setBodyArea(''); setStep('body'); }} color="#00E5FF">
          ...
          {step === 'body' && (
            <motion.div key="body" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-2">
              <p className="text-slate-400 text-sm mb-1">Which area is affected?</p>
              <p className="text-slate-600 text-xs mb-4">Suggested for {getSymptomLabel(category, symptom)}</p>
              <div className="grid grid-cols-2 gap-2">
                {currentBodyAreas.map((area) => (
          ...
          {step === 'age' && (
            <motion.div key="age" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-6">
              ...
              <div className="space-y-2">
                <div className="text-slate-500 text-xs">Tap any detail that applies</div>
                <div className="flex gap-2 flex-wrap">
                  {detailSuggestions.map((detail) => (
                    <button
                      ...
                      className="px-3 py-1.5 rounded-lg text-xs text-slate-300 hover:text-cyan-200 transition-all"
