  ...
  return (
    <div className="relative w-full h-screen overflow-hidden" style={{ background: '#050816' }}>
      ...
      {isGlobe && (
        <>
          <motion.div
            className="fixed top-[76px] right-6 z-30 hidden w-[220px] flex-col gap-3 xl:flex"
          ...
          <motion.div
            className="fixed bottom-[72px] left-1/2 z-30 -translate-x-1/2"
      ...
      <AnimatePresence>
        {(pipelineState.status === 'fetching' || pipelineState.status === 'analyzing') && (
          <motion.div
            className={`fixed left-1/2 z-40 flex -translate-x-1/2 items-center gap-3 rounded-full px-5 py-2 ${isGlobe ? 'top-[72px]' : 'bottom-16'} glass`}
            initial={{ y: 20, opacity: 0 }}
            animate={{ y: 0, opacity: 1 }}
            exit={{ y: 20, opacity: 0 }}
          >
