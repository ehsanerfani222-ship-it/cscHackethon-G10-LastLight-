...
export function SafeZonesPanel() {
  const { facilities, setFacilities, userLocation, setUserLocation, crises, selectedCrisis } = useStore();
  const [filter, setFilter] = useState<FilterType>('all');
  const [isLoadingLoc, setIsLoadingLoc] = useState(false);
  const [isLoadingFac, setIsLoadingFac] = useState(false);

  const crisis = selectedCrisis ?? crises.sort((a, b) => b.severity - a.severity)[0];
  ...
  const getLocation = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      ...
      () => { toast.error('Location access denied'); setIsLoadingLoc(false); }
    );
  };
  ...
  return (
    <div className="flex flex-col h-full" style={{ background: '#050816' }}>
      <div className="px-4 py-4 flex-shrink-0" style={{ borderBottom: '1px solid rgba(0,229,255,0.1)', background: 'rgba(5,8,22,0.95)' }}>
        ...
        {/* Active crisis alert */}
        {crisis && (
          <div className="mb-3 p-3 rounded-xl" style={{ background: 'rgba(255,59,92,0.08)', border: '1px solid rgba(255,59,92,0.2)' }}>
            <div className="text-xs text-red-400 font-semibold mb-1">⚠️ Active Threat Nearby</div>
            <div className="text-white text-sm font-medium">{crisis.title}</div>
            <div className="text-slate-400 text-xs">{crisis.location} • Severity {crisis.severity.toFixed(1)}/10</div>
          </div>
        )}
