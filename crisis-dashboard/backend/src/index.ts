...
import { ensureTodayHasCrises, getCrises } from './services/crisis.service';
...
// Pipeline broadcasts real-time status to all connected clients
...
app.post('/api/pipeline/scan', async (_req: Request, res: Response) => {
  ...
  // Fire and forget — real-time updates via WebSocket
...
server.listen(PORT, async () => {
  ...
  if (hasKey) {
    ...
    console.log('ℹ️  No API key — loading real earthquake data + seed crises');
    const empty = await isTodayEmpty();
    if (empty) await seedDatabase();
    // Still overlay real USGS earthquakes even without AI
    await runAIPipeline(); // this gracefully falls back
    broadcastUpdate();
  }
});
